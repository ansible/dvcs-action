...
def delete_previous_comment_if_needed(comments_url) -> None:
    ...
    for comment in comments.json():
        if comment['user']['login'] == 'ansible':
            previous_comment = comment
    
    # If we already made a comment, remove it
    if previous_comment:
        print("Deleting previous comment ... ", end="")
        response = requests.delete(previous_comment['url'], headers=http_headers)
...
def make_decisions(pr_title_jira: Optional[str], possible_commit_jiras: list[str], source_branch_jira: Optional[str]) -> str:
    decisions = ["DVCS PR Check Results:"]
    # Now make th decisions if this is in good order....
    if not pr_title_jira:
        # If there is no PR title JIRA report bad
        decisions.append(f"* {bad_icon} Title: PR title does not start with a JIRA number or {_NO_JIRA_MARKER}")
    else:
    ...
    if not source_branch_jira:
        ...
        decisions.append(f"* {bad_icon} Source Branch: The source branch of the PR does not start with a JIRA number or {_NO_JIRA_MARKER}")
    else: 
        # Report the source branch is good
        decisions.append(f"* {good_icon} Source Branch: JIRA number {source_branch_jira}")
    
    if pr_title_jira is not None and pr_title_jira != source_branch_jira:
        # If we have source and title JIRAS and there is a mismatch between the title and source branch JIRAs report the mismatch
        decisions.append(f"* {bad_icon} Mismatch: The JIRAs in the source branch and title do not match!")
    
    if len(possible_commit_jiras) == 0:
        # If we got no commit JIRAS the commits are bad
        decisions.append(f"* {bad_icon} Commits: No commits with a JIRA number or {_NO_JIRA_MARKER} found!")
        ...
        if source_branch_jira is None:
            ...
            if source_branch_jira in possible_commit_jiras:
                decisions.append(f"* {good_icon} Commits: At least on JIRA number in commit messages match the other JIRA numbers")
    else:
